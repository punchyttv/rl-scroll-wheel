...
function getImage(key) {
  ...
  try {
    if (!fs.existsSync(folder)) {
      console.error('Rank folder missing:', folder);
      return '';
    }
    ...
    const found = files.find(file => {
      ...
      return filename === String(key).toLowerCase();
    });

    return found ? `/assets/ranks/${found}` : '';
  } catch (error) {
    console.error('Unable to load rank image:', error);
    return '';
  }
}
...
function applyEvent(type, username, amount) {
  ...
  if (type === 'follow') {
    viewer.totalFollows += 1;
  }

  if (type === 'gift' || type === 'rose') {
    viewer.totalGifts += 1;
  }
