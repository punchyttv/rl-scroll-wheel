...
function getImage(key) {
  try {
    ...
    const found = files.find(file => {
      ...
      return fileNameWithoutExtension.toLowerCase() === String(key).toLowerCase();
    });

    return found ? `/${encodeURIComponent(found)}` : '';
  } catch (error) {
    console.error(`Could not find rank image for "${key}":`, error.message);
    return '';
  }
}
...
function applyEvent(type, username, amount) {
  ...
  const payload = {
    ...
    addXp: add,
    xp: viewer.xp,
    beforeRank: before.name,
    beforeKey: before.key,
    beforeImg: getImage(before.key),
    rank: after.name,
    rankKey: after.key,
    rankImg: getImage(after.key),
    nextRank: after.next?.name || null,
    nextXp: after.next?.xp || null,
    progress: after.next
      ? Math.round(
          ((viewer.xp - after.xp) / (after.next.xp - after.xp)) * 100
        )
      : 100,
...
app.get('/player/:username', (req, res) => {
  ...
  const viewer = db[username];

  if (!viewer) {
    return res.status(404).json({ error: 'Player not found' });
  }

  const rank = rankForXp(viewer.xp);

  res.json({
    ...viewer,
    rank: rank.name,
    rankImg: getImage(rank.key),
    nextRank: rank.next?.name || null,
    nextXp: rank.next?.xp || null
  });
});
...
app.get('/test', (req, res) => {
  ...
  const payload = applyEvent(
    req.query.type || types[Math.floor(Math.random() * types.length)],
    req.query.user || users[Math.floor(Math.random() * users.length)],
    req.query.xp
  );
